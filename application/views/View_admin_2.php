...
<div class="rounded-3 py-5 px-4 px-md-5 mb-5">
  <div class="text-center mb-5">
    <div class="feature bg-primary bg-gradient text-white rounded-3 mb-3"><i class="bi bi-envelope"></i></div>
        <h1 class="fw-bolder">Registrar un Editor</h1>
    </div>
...
function registrar_columnista(){
  $.ajax({
    method:"POST",
    url:"<?php echo site_url("Ctrl_admin_2/registrar");?>",
    data:{
...
function modificar(){
  $.ajax({
    method:"POST",
    url:"<?php echo site_url("Ctrl_admin_2/modificar");?>",
    data:{
...
function editar(id_usuario_e,user,password,fecha_creacion,id_editor,id_usuario_adm){
  $("#txb_id_usuario_col").val(id_usuario_e),
  $("#txb_usuario").val(user);
  $("#txb_password").val(password);
  $("#txb_fecha_c").val(fecha_creacion);  
  $("#txb_columnista").val(id_editor);
...
function listar_columistas(){
  $.ajax({
    method:"POST",
    url:"<?php echo site_url("Ctrl_admin_2/tabla_columinstas");?>",
    data:{
...
function crear_tabla_columnistas(columnistas){  
    if(columnistas.length >0){
      ...
      for(i=0;i<columnistas.length;i++){
        ...
        tabla_dinamica+="<td>"+columnistas[i].id_editor+"</td>";
        tabla_dinamica+="<td>";
        tabla_dinamica+="<button class='btn btn-outline-danger btn-lg' onclick=\"editar('"+
        columnistas[i].id_usuario_e+
        "','"+columnistas[i].user+
        "','"+columnistas[i].password+

        "','"+columnistas[i].fecha_creacion+
        "','"+columnistas[i].id_editor+
        "','"+columnistas[i].id_usuario_adm+
        "')\">MODIFICAR</button>";
        tabla_dinamica+="<td>";
        tabla_dinamica+="<button class='btn btn-outline-danger btn-lg' onclick=\"eliminar('"+columnistas[i].id_usuario_e+"')\">ELIMINAR</button>";
        tabla_dinamica+="</td>";
        tabla_dinamica+="</tr>";
      }
...
function eliminar(id_usuario){
  $.ajax({
    method:"POST",
    url:"<?php echo site_url("Ctrl_admin_2/eliminar_registro");?>",
    data:{
...
function Buscar(){
  $.ajax({
    method:"POST",
    url:"<?php echo site_url("Ctrl_admin_2/buscar");?>",
    data:{
      vname_user:$("#txb_name_user").val()
    },
    success: function(columnistas){
      crear_tabla_columnistas(columnistas);
      //crear_tabla_monto(noticias);/////////
    },
    dataType:'json'
});
